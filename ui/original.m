...
% ORIGINAL Application M-original for original.fig
%    FIG = ORIGINAL launch original GUI.
%    ORIGINAL('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.0 06-Jun-2005 21:19:43
...
[filename, pathname] = uigetfile( {'*.dat', 'All Data Files (*.dat)'; '*.*','All Files (*.*)'},'Select Input data file');
...
delimeter = ' ';
if (handles.popupDelimeter == 'Comma')
    delimeter = ',';
else if (handles.popupDelimeter == 'Space')
    delimeter = ' ';
else if (handles.popupDelimeter == 'Tab')
    delimeter = '\t';
end
end
end
handles.Delimeter = delimeter;
guidata(h,handles);
